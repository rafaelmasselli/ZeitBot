...
export class BrazilIndeedStrategy implements BaseNewsStrategyRepository {
  public mapItemToNews(item: any): News {
    const news = new News();
    ...
    news.date = new Date();
    news.enterprise = INewsPlatform.BRAZIL_INDEED;
    news.topics = item.category ? [item.category] : [];
    return news;
  }

  public parse(html: string): News[] {
  ...
  public convertHtmlToText(html: string, news: News): News {
    ...
    const dateText = $(".jeg_meta_date a").text().trim();
    if (dateText) {
      try {
        newUpdated.date = new Date(dateText);
      } catch (error) {
        console.log(error);
      }
    }
  ...
  public getDetailsNews(html: string, news: News): News {
    try {
      const $ = cheerio.load(html);
